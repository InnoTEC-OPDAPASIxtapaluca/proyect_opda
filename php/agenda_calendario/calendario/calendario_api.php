...
/**
 * calendario_api.php - API para gestión del calendario
...
class CalendarioAPI {
    ...
    private function getUsuariosPorArea() {
        try {
            ...
            $query = "SELECT 
                        ...
                        ui.identificador,
                        ui.Nombre as nombre,
                        ui.Apellido_Paterno as apellido_paterno,
                        ui.Apellido_Materno as apellido_materno
                      FROM usuarios_internos ui
                      INNER JOIN areas a ON ui.area_id = a.id_area
                      WHERE ui.identificador IS NOT NULL";
            
            if (!empty($identificadorActual)) {
                $query .= " AND ui.identificador != :identificador";
            }
            
            $query .= " ORDER BY a.area ASC, ui.Nombre ASC";
            ...
            // Convertir a array indexado (eliminar las keys del área)
        ...
        try {
            ...
            foreach ($eventos as &$evento) {
                if (!empty($evento['archivos'])) {
                    $evento['archivos'] = $this->blobToJson($evento['archivos'], $evento['id_evento'], 'calendario');
                }
    ...
    private function blobToJson($blob, $id_evento = null, $origen = 'calendario') {
